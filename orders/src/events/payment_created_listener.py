...
class PaymentCreatedListener(Listener):
    subject = Subjects.PaymentCreated
    queueGroupName = QueueGroupName.PaymentCreatedListeners
    ...
    async def onMessage(self, data, msg):
        ...
        q = select(Order).where(
            and_(Order.uuid == data["orderId"], Order.version_id == data["version_id"] - 1))
        try:
            try:
